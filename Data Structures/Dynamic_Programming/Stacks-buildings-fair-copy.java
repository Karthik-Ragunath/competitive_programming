...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        long[] buildingheights=new long[numberofbuildings];
        int j=0;
        long amountofmemoryneeded=(numberofbuildings*(numberofbuildings+1))/2;
        long maximumheight=0;
        long[] dynamicsolution=new long[(int)amountofmemoryneeded];
        for(int i=0;i<numberofbuildings;i++)
        {
            buildingheights[i]=in.nextLong();
            for(j=0;j<=i;j++)
            {
                if(j==i)
                {

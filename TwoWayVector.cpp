...
ll returnLargestArea()
{
    ...
    for(ll i = 0; i < islandNumber; i++)
    {
        ...
        unordered_map< ll, pair< ll, vector< ll > > > islandValueSizePair;
            ...
            else
            {
                if(!visitedIsland[childIslandIndex])
                {
                    ...
                    if(previousIslandValuesIter == previousIslandValues.end())
                    {
                        ...
                        unordered_map< ll, pair< ll, vector< ll > > >::iterator unorderedIter = islandValueSizePair.find(childIslandValue);
                        if(unorderedIter != islandValueSizePair.end())
                        {
                            vector< ll > correspondingChildSet = unorderedIter -> second.second;
                            vector< ll >::iterator corresChildIter = correspondingChildSet.begin();
                            bool alreadyVisited = false;
                            for(; corresChildIter != correspondingChildSet.end(); corresChildIter++)
                            {
                                if(visitedIsland[*corresChildIter])
                                {
                                    alreadyVisited = true;
                                    break;
                                }
                            ...
                            if(!alreadyVisited)
                            {
                                ll alreadyFoundSize = unorderedIter -> second.first;
                                ll newValue = islandArea + alreadyFoundSize;
                                islandValueSizePair.erase(unorderedIter);
                                correspondingChildSet.push_back(childIslandIndex);
                                islandValueSizePair.insert(make_pair(childIslandValue, make_pair(newValue, correspondingChildSet)));
                            }
                        }
                        else
                        {
                            vector< ll > newSet;
                            newSet.push_back(childIslandIndex);
                            islandValueSizePair.insert(make_pair(childIslandValue, make_pair(islandArea, newSet)));
                        }
                    }
                    else
                    {
        ...
        unordered_map< ll, pair< ll, vector< ll > > >::iterator unorderedIter = islandValueSizePair.begin();
        for(; unorderedIter != islandValueSizePair.end(); unorderedIter++)
        {
            ll value = unorderedIter -> second.first;

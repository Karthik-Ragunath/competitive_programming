...
void calculateFactorial()
{
	...
	inverseFactorialVector[5000] = modularInverse(factorialVector[5000]) % mod;
    for(ll i = 5000; i > 0; i--)
    {
...
ll nCr(ll n, ll r)
{
    ...
    return combinationsCounter % mod;
}
...
ll combinationsCount(ll smaller, ll larger, ll N, ll K)
{
    ...
    for(ll i = 1, j = maxPossibleOnOneSide; i <= (K - 2); i++, j--)
    {
        if(i <= smaller)
        {
            ...
            {
                // cout << "n: " << smaller << " r: " << i;
                ll solution1 = nCr(smaller, i) % mod;
                // cout << " solution: " << solution1 << endl;
                // cout << "n: " << larger << " r: " << j;
                ll solution2 = nCr(larger, j) % mod;
    ...
    for(ll n = 0; n < testcases; n++)
    {
        ...
        for(ll i = 1, j = (N - 2); i < (N - 1); i++, j--)
        {
            ...
            else
            {
                ...
                {
                    ll numberOfCombinations = combinationsCount(numberOfTermsOnLeft, numberOfTermsOnRight, N, K) % mod;
                    solutionVect[numberOfTermsOnLeft] = numberOfCombinations % mod;
                    solutionVect[numberOfTermsOnRight] = numberOfCombinations % mod;
                }
                else
                {
                    ll numberOfCombinations = combinationsCount(numberOfTermsOnRight, numberOfTermsOnLeft, N, K) % mod;

...
public:
    string numberToWords(int num) {
        ...
        */
        ...
        {
            splitted_strings_reversed.push_back(number_string_reversed.substr(j, j + 3));
            j = (i + 1) * 3;
        }
        if(string_size % 3 != 0)
        {
            number_of_strings += 1;
            splitted_strings_reversed.push_back(number_string_reversed.substr(string_size - string_size % 3, string_size % 3));
        }
        ...
        for(ll i = 0; i < number_of_strings; i++)
        {
            string internal_solution_string = "";
            string val = splitted_strings_reversed[i];
            for(ll j = 0; j < val.length(); j++)
            {
                int k = val[j];
                if(split_intermediated[k][j] != "")
                {
                    if(internal_solution_string != "")
                    {
                        internal_solution_string = split_intermediated[k][j] + " " + internal_solution_string;
                    }
                    else
                    {
                        internal_solution_string = split_intermediated[k][j];
                    }
                }   
            }
            if(internal_solution_string != "")

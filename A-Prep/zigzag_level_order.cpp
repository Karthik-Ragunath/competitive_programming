    ...
    public:
        ...
        vector<vector<int>> zigzagLevelOrder(TreeNode* root) 
        {
            ...
            /*
           	...
           	while(not bfs.empty())
           	{
           		pair< TreeNode*, int > pair_val = bfs.front();
           		bfs.pop();
           		if(pair_val.second % 2 == 0)
           		{
           			if(pair_val.first -> right)
           			{
           				bfs.push(make_pair(pair_val.first -> right, pair_val.second + 1));
           			}
           			if(pair_val.first -> left)
           			{
           				bfs.push(make_pair(pair_val.first -> left, pair_val.second + 1));
           			}
           		}
           		else
           		{
           		...
           		zig_zag_levels[pair_val.second].push_back(pair_val.first -> val);
           	}
            */
